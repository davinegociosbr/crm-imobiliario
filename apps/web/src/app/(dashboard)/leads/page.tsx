  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
        <div className="flex items-center gap-2 flex-1">
          ...
          <select
            ...
            className="py-2 px-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-900"
          >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <a
            ...
            className="p-2 rounded-lg border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
          ...
          <button
            ...
            disabled={importing}
            className="p-2 rounded-lg border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors disabled:opacity-60"
                  ...
                  <tr key={lead.id} className="border-b last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => setAddToPipelineLead(lead)} className="p-1.5 rounded hover:bg-purple-50 text-purple-600" title="Adicionar ao Funil">
                          <Kanban className="w-4 h-4" />
                        </button>
                        <Link href={`/leads/${lead.id}`} className="p-1.5 rounded hover:bg-blue-50 text-blue-600">
                          <Eye className="w-4 h-4" />
                        </Link>
                        <button onClick={() => deleteMutation.mutate(lead.id)} className="p-1.5 rounded hover:bg-red-50 text-red-500">
      ...
      <div className="md:hidden space-y-2">
        {isLoading && <p className="text-center py-12 text-slate-400">Carregando...</p>}
        {!isLoading && leads.length === 0 && <p className="text-center py-12 text-slate-400">Nenhum lead encontrado</p>}
        {leads.map((lead: any) => {
          const statusInfo = LEAD_STATUS[lead.status as keyof typeof LEAD_STATUS];
          const phone = lead.phone || lead.whatsapp;
          return (
            <div key={lead.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <p className="font-semibold text-slate-800 dark:text-white truncate">{lead.name}</p>
                  {lead.city && <p className="text-xs text-slate-400">{lead.city}{lead.state ? `, ${lead.state}` : ''}</p>}
                </div>
                <span className={`shrink-0 inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${statusInfo?.color}`}>
                  {statusInfo?.label || lead.status}
                </span>
              </div>
              <div className="flex flex-wrap gap-2 mb-3">
                {phone && (
                  <a href={`tel:${phone}`} className="flex items-center gap-1 text-xs text-blue-600 bg-blue-50 dark:bg-blue-950/30 px-2 py-1 rounded-lg">
                    <Phone className="w-3 h-3" />{phone}
                  </a>
                )}
                {lead.whatsapp && (
                  <a href={`https://wa.me/55${lead.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-green-700 bg-green-50 dark:bg-green-950/30 px-2 py-1 rounded-lg">
                    <MessageCircle className="w-3 h-3" />WhatsApp
                  </a>
                )}
                {lead.email && (
                  <a href={`mailto:${lead.email}`} className="flex items-center gap-1 text-xs text-slate-600 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg truncate max-w-[160px]">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <button onClick={() => setAddToPipelineLead(lead)} className="p-2 rounded-lg bg-purple-50 text-purple-600" title="Funil">
                    <Kanban className="w-4 h-4" />
                  </button>
                  <Link href={`/leads/${lead.id}`} className="p-2 rounded-lg bg-blue-50 text-blue-600">
                    <Eye className="w-4 h-4" />
                  </Link>
                  <button onClick={() => deleteMutation.mutate(lead.id)} className="p-2 rounded-lg bg-red-50 text-red-500">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
